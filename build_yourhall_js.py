...
def build(cfg):
    ...
    cards = _cards(cfg, players, full)
    _why_bundle(cfg, config, players, cards)
...
def _why_bundle(cfg, config, players, cards):
    """One namespaced file per side for the Who's Better page, so BOTH sides
    can load in the same document (the yourhall-*-data.js files each declare a
    bare `const YH_CONFIG` / `YH_PLAYERS` and would collide). Only the fields
    why-embed.js actually reads: id / name / hof / per-season matrix, plus the
    career cards. Everything is recomputed from `s`, so `z` / `nel` / `wl` are
    dropped."""
    side = cfg["side"]
    slim = [{"id": p["id"], "n": p["n"], "hof": p["hof"], "s": p["s"]} for p in players]
    bundle = {"config": config, "players": slim, "cards": cards}
    out = (
        f"// Head-to-head data for the Who's Better page ({side}): config + the\n"
        f"// full-pool per-season z-matrix + career cards, under a namespaced global\n"
        f"// so both sides load together. Generated by build_yourhall_js.py.\n"
        "window.WHY_DATA = window.WHY_DATA || {};\n"
        f"window.WHY_DATA.{side} = " + json.dumps(bundle, separators=(",", ":")) + ";\n"
    )
    (HERE / f"js/why-{side}.js").write_text(out)
    print(f"js/why-{side}.js: {len(slim)} {side} + {len(cards)} cards, {len(out) / 1024:.0f} KB")
